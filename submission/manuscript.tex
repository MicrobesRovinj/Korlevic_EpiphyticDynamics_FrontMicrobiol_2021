...
(\autoref{supp-nseq_notus}), through rarefaction resulting in 17,203
different OTUs that ranged from 376 to 2,027 OTUs per sample
...
found for \emph{C. cylindracea} (mixed {[}Number of OTUs, 1,689.6 ±
136.3 OTUs{]} and monospecific {[}Number of OTUs, 1,739.3 ± 156.0
OTUs{]}), middle values for \emph{C. nodosa} (Number of OTUs, 1,055.3 ±
217.2 OTUs) and lower values for picoplankton communities in the ambient
seawater (Number of OTUs, 526.5 ± 141.2 OTUs)
...
These OTUs also made a high proportion of total sequences (40.1 -- 51.5
...
Autumn/Winter samples (ANOSIM, R = 0.55, \emph{p} \textless{} 0.01). For
...
supported (ANOSIM, R = 0.33, \emph{p} \textless{} 0.01)
(\autoref{pcoa}).
...
were accounting for a high proportion of sequences (\(\leq\) 51.5
