...
(\autoref{supp-nseq_notus}), through rarefaction resulting in 17,022
different OTUs that ranged from 376 to 2,013 OTUs per sample
...
found for \emph{C. cylindracea} (mixed {[}Number of OTUs, 1,692.4 ±
123.7 OTUs{]} and monospecific {[}Number of OTUs, 1,727.2 ± 156.3
OTUs{]}), middle values for \emph{C. nodosa} (Number of OTUs, 1,049.2 ±
206.9 OTUs) and lower values for picoplankton communities in the ambient
seawater (Number of OTUs, 529.4 ± 143.8 OTUs)
...
(\autoref{shared}). In addition, only 0.5 -- 1.0 \si{\percent} of OTUs
from each surface associated community were found at every time point.
These OTUs also made a high proportion of total sequences (41.6 -- 53.0
...
were accounting for a high proportion of sequences (\(\leq\) 53.0
