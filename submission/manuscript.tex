...
(\autoref{supp-nseq_notus}), through rarefaction resulting in 17,222
different OTUs that ranged from 364 to 2,005 OTUs per sample
...
found for \emph{C. cylindracea} (mixed {[}Number of OTUs, 1,693.8 ±
137.1 OTUs{]} and monospecific {[}Number of OTUs, 1,733.2 ± 161.8
OTUs{]}), middle values for \emph{C. nodosa} (Number of OTUs, 1,051 ±
201.5 OTUs) and lower values for picoplankton communities in the ambient
seawater (Number of OTUs, 531.2 ± 140.7 OTUs)
...
(\autoref{shared}). In addition, only 0.4 -- 1.1 \si{\percent} of OTUs
from each surface associated community were found at every time point.
These OTUs also made a high proportion of total sequences (40.2 -- 53.1
...
\emph{FEMS Microbiol Ecol} \textbf{83}: 1--16.
...
V., Leisch, F., and Peng, R.D. (eds). New York: Chapman; Hall/CRC, pp.
3--32.
...
definitive guide, 1st ed. Boca Raton, Florida: Chapman; Hall/CRC.
