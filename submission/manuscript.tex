...
(\autoref{supp-nseq_notus}), through rarefaction resulting in 16,963
different OTUs that ranged from 365 to 2,019 OTUs per sample
...
1,686.9 ± 135.8 OTUs{]} and noninvaded {[}Number of OTUs, 1,742.8 ±
152.4 OTUs{]}), middle vales for \emph{Cymodocea nodosa} (Number of
OTUs, 1,047.7 ± 212.1 OTUs) and lower values for picoplankton
communities in the surrounding seawater (Number of OTUs, 528.4 ± 144.5
...
showed a similar pattern (ANOSIM, R = 0.54, \emph{p} \textless{} 0.01),
...
between Summer and Autumn/Winter/Spring samples (ANOSIM, R = 0.30,
\emph{p} \textless{} 0.01) (\autoref{pcoa}).

The microbial taxonomic composition of both, macrophyte associated and
seawater communities, was dominated by \emph{Bacteria} (99.1 ± 2.1
\si{\percent}) over \emph{Archaea} (0.9 ± 2.1 \si{\percent})
(\autoref{community}). Three phyla were characteristic for
\emph{Archaea}: \emph{Crenarchaeota}, \emph{Nanoarchaeota} and
\emph{Thermoplasmatota}. Their contribution was larger only in seawater
communities during December 2017 (\autoref{community}). Higher relative
abundances of chloroplast related sequences were only observed in
surface associated communities, with higher values in Autumn/Winter
(37.2 ± 11.2 \si{\percent}) in comparison to Spring/Summer (20.9 ± 9.7
\si{\percent}) (\autoref{supp-chloroplast}). Generally, at higher
taxonomic ranks (phylum-class) epiphytic and seawater microbial
...
consisted of and sequences without known relatives within
\emph{Cyanobacteriales} and \emph{Cyanobacteriia} (\autoref{cyano}). In
addition, seasonal changes in surface associated communities were
observed with \emph{Cyanobacteriales} and \emph{Cyanobacteriia} no
relative sequences comprising larger proportions in autumn and winter
and \emph{Acrophormium}, \emph{Phormidesmis} and no relative
\emph{Nodosilineaceae} and in spring and summer (\autoref{cyano}).
...
uncultured \emph{Balneolaceae} and \emph{Formosa}. In contrast, surface
associated communities were characterized by \emph{Lewinella},
\emph{Portibacter}, \emph{Rubidimonas}, no relative
\emph{Saprospiraceae}, uncultured \emph{Saprospiraceae}, no relative
\emph{Flavobacteriaceae} and uncultured \emph{Rhodothermaceae}. Surface
associated \emph{Bacteroidota} communities were very diverse as could be
observed in the the high proportion of taxa that grouped as other
...
no relative \emph{Alphaproteobacteria} and no relative
\emph{Hyphomonadaceae}. Representatives of no relative
\emph{Rhodobacteraceae} were comprising in on average 40.6 ± 23.2
\si{\percent} of all alphaproteobacterial sequences from the epiphytic
community (\autoref{alpha}).
...
similarly to \emph{Bacteroidota}, a large proportion of the surface
associated community was grouped as other \emph{Gammaproteobacteria}
indicating high diversity within this group (\autoref{gamma}).
